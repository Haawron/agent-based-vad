...
def pretrain_internvideo2_1b_patch14_224(config):
    ...
    if config.vision_encoder.pretrained is not None:
        ...
        state_dict = {name.split('vision_encoder.')[-1]: tensor for name, tensor in state_dict['module'].items() if name.startswith('vision_encoder.')}
        # interpolate_pos_embed_internvideo2(state_dict, model, orig_t_size=8)  # 8 맞음? config에 없는데?
        interpolate_pos_embed_internvideo2(state_dict, model, orig_t_size=1)
